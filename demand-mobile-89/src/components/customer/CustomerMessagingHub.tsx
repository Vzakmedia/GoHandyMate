...
export const CustomerMessagingHub = () => {
  const { user } = useAuth();
  ...
  const fetchMessageThreads = useCallback(async () => {
    ...
    try {
      // Get all jobs for the customer that have messages
      ...
      // Get handyman profiles for the jobs
      const handymanIds = [...new Set(jobsWithMessages?.map(job => job.assigned_to_user_id).filter(Boolean))];

      const { data: handymenProfiles, error: profilesError } = await supabase
        .from('profiles')
        .select('id, full_name')
        .in('id', handymanIds);
      ...
      // Process the data to create message threads
      const threads: MessageThread[] = [];

      for (const job of jobsWithMessages || []) {
        if (!job.job_messages || job.job_messages.length === 0) continue;

        const handyman = handymenProfiles?.find(h => h.id === job.assigned_to_user_id);
        if (!handyman) continue;

        // Sort messages by date to get the latest one
        const sortedMessages = job.job_messages.sort(
          (a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()
        );

        const lastMessage = sortedMessages[0];

        // Count unread messages (messages from handyman that customer hasn't read)
        ...
        threads.push({
          ...
          handyman_id: job.assigned_to_user_id as string,
          handyman_name: handyman.full_name as string,
          last_message: {
  ...
  const handleOpenMessaging = (thread: MessageThread) => {
    ...
    <div className="flex flex-col space-y-8">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Connect with your pros and neighbors</p>
        </div>
      ...
      {/* Pill-style Filter/Action Bar (Optional but good for consistency) */}
      <div className="flex items-center gap-2 p-1.5 bg-white/50 backdrop-blur-sm border border-black/5 rounded-[2rem] w-fit">
        <Button
          variant="default"
          size="sm"
          className="rounded-full px-6 bg-[#166534] text-white transition-all duration-300"
        >
        ...
        <div className="p-8">
          {messageThreads.length === 0 ? (
            <div className="text-center py-20">
              ...
              <p className="text-slate-500 text-sm max-w-[240px] mx-auto">Messages with your handymen will appear here once you've booked a service.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {messageThreads.map((thread) => (
                <div
                  ...
                  onClick={() => handleOpenMessaging(thread)}
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-3">
                        ...
                        <div>
                          <h4 className="font-bold text-slate-900 leading-tight">{thread.handyman_name}</h4>
                          <span className="text-[10px] font-black uppercase tracking-widest text-[#166534]">{thread.job_title}</span>
                        </div>
                      </div>

                      <p className="text-sm text-slate-600 line-clamp-2 pl-13 mb-3">
                      ...
                      <div className="flex items-center gap-4 pl-13">
                        ...
                        <Badge className={`rounded-full px-3 py-0.5 text-[10px] font-black uppercase tracking-widest border-none ${getStatusColor(thread.job_status)}`}>
      ...
      {selectedThread && (
        <JobMessagingModal
          ...
          otherParticipantId={selectedThread.handyman_id}
          otherParticipantName={selectedThread.handyman_name}
        />
      )}
    </div>
  );
